...
const { getTimestamp } = require("./middleware");
// const { c2b } = require("./mpesa");

exports.youtubePayment = async (req, res) => {
  try {
    const { phone, amount } = req.body;

    const cbSucess = (resp) => {
      res.status(202).send({
        message: "pagamento bem succedido",
      });
    };
    const cbBug = (error) => {
    ...
    c2b({ amount: String(amount), number: phone }, cbSucess, cbBug);
  } catch (error) {
    console.log(error);
